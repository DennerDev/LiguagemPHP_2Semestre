	...
	<body>
		...
		<?php
			...
			*/
			...
			// 3 - Colocar numa variável o comando de seleção de dados
			$comandoSQL = "SELECT * FROM pets ORDER BY nome";
			...
			*/
			...
			for($n=0; $n<$linhas; $n++)
			{
				...
				// Guardar os dados do pet lido em variáveis
				$id			= $dados["id"];
				$nome		= $dados["nome"];
				$tipo		= $dados["tipo"];
				$sexo		= $dados["sexo"];
				...
				echo "	<td>$idade</td>";
				echo "	<td>$nomeDono</td>";
				echo "	<td> <a href='excluirPet.php?id=$id&nome=$nome'> <img src='imgs/cancelar.png' alt='Excluir Pet $nome' title='Excluir Pet $nome'> </a> </td>";
				echo "</tr>";
			}
		...
		?>
		<hr>
		Clique <a href='novoPet.html'>aqui</a> para incluir um novo Pet
	</body>
</html>
